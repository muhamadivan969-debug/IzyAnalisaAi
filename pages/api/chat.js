export default async function handler(req, res) {
  ...
  try {
    ...
    if (context) {
      ...
      dataSection += `Waktu: ${context.waktu || "Terbaru"}\n`;
      dataSection += `IHSG: ${context.ihsg || "N/A"} (${context.ihsgPersen || "N/A"})\n`;
      if (context.stockCode) {
        dataSection += `Saham: ${context.stockCode} (${context.stockName || ""})\n`;
        dataSection += `Harga Terakhir: ${context.close || "N/A"} (${context.changePercent || "0"}%)\n`;
      }
      dataSection += "======================================\n";
    }
...
7. Selalu akhiri dengan: "DYOR (Do Your Research) - ini bukan saran finansial."`;
    ...
    // Model yang ditingkatkan untuk Google Generative Language Interactions API
